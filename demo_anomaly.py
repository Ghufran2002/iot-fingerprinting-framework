...
sep()
print("  DEMO 2 — Anomaly Detection & Attack Identification")
sep()
...
sep("-")
r = post("/anomaly/score", {"features": CAMERA_NORMAL, "device_type": "smart_camera", "source_ip": "192.168.1.45"})
anomaly_result("Smart Camera — Regular HTTPS video stream", r["anomaly_score"], r["is_anomalous"], r["severity"])
print("\n  Normal traffic: 400 pkt/s, 28MB upload, ratio = 2. System says NORMAL.")
time.sleep(1)
...
exfil.update({"byte_count": 2100000000, "byte_rate": 36000000,
              "upload_bytes": 1680000000, "download_bytes": 14000000,
              "upload_download_ratio": 120.0})
r = post("/anomaly/score", {"features": exfil, "device_type": "smart_camera", "source_ip": "192.168.1.45"})
anomaly_result("Smart Camera — Data Exfiltration", r["anomaly_score"], r["is_anomalous"], r["severity"])
print("\n  Camera uploading 1.68 GB, downloading only 14 MB.")
print("  Upload:download ratio = 120 (normally 2). Mirai botnet exfiltration. ALERT.")
time.sleep(1)
...
r = post("/anomaly/score", {"features": port_scan, "device_type": "smart_thermostat", "source_ip": "192.168.1.22"})
anomaly_result("Smart Thermostat — Port Scan / Network Reconnaissance", r["anomaly_score"], r["is_anomalous"], r["severity"])
print("\n  Thermostat normally contacts 1 IP on port 1883 only.")
print("  Here scanning 1,200 ports across 250 IPs. Port entropy = 9.5 (normally 0.3).")
time.sleep(1)

# TEST 4 — Combined /analyze
sep("-")
print("  TEST 4 — Combined: Fingerprint + Anomaly in One Call (POST /analyze)")
sep("-")
print("\n  No device_type given — system auto-detects device first.\n")
exfil_analyze = dict(CAMERA_NORMAL)
exfil_analyze.update({"byte_count": 2100000000, "byte_rate": 36000000,
                      "upload_bytes": 1680000000, "download_bytes": 14000000,
                      "upload_download_ratio": 120.0})
r = post("/analyze", {"features": exfil_analyze, "source_ip": "192.168.1.45"})
fp = r["fingerprint"]
an = r["anomaly"]
print(f"  --- Fingerprinting Result ---")
print(f"  Device Detected : {fp['device_type'].upper()}")
print(f"  Confidence      : {fp['confidence']:.2%}")
print(f"  Is Known Device : {fp['is_known']}")
anomaly_result("Auto-detected Camera — Data Exfiltration", an["anomaly_score"], an["is_anomalous"], an["severity"])
print(f"\n  /analyze fingerprinted device as smart_camera ({fp['confidence']:.0%} confidence)")
print(f"  then ran anomaly detection automatically. Both results in one API call.")
...
print("  All 3 attack types detected with correct severity.")
sep()
